...
Fusion::Fusion(MPU9250& sensRef)  : sensor(sensRef){}
...
void Fusion::update() {
    ...
    yaw   = atan2(2.0f * (*(q+1) * *(q+2) + *q
                  * *(q+3)), *q * *q + *(q+1)
                  * *(q+1) - *(q+2) * *(q+2) - *(q+3)
                                                                                                                                            * *(q+3));
    pitch = -asin(2.0f * (*(q+1) * *(q+3) - *q
                  * *(q+2)));
...
void Fusion::MadgwickQuaternionUpdate()
{
    float ax=sensor.ax;
    float ay=sensor.ay;
    float az=sensor.az;
...
void Fusion::MahonyQuaternionUpdate()
{
    float ax=sensor.ax;
    float ay=sensor.ay;
    float az=sensor.az;
